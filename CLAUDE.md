# CLAUDE.md

このファイルは、Claude Code (claude.ai/code) がこのリポジトリで作業する際のガイダンスを提供します。

## プロジェクト概要

Astroで構築した技術ブログ。プログラミングの知識や学習経験を共有するためのサイト。

## コマンド

### 開発

- `pnpm dev` - 開発サーバーを起動
- `pnpm build` - 静的サイトをビルド
- `pnpm preview` - ビルド結果をローカルでプレビュー

### プロジェクト構成

- `src/` - Astroのソースコード
- `posts/` - ブログ記事ディレクトリ（Markdown形式）
- `docs/` - 作業ログとドキュメント
- `todo.md` - タスク管理

## 記事の執筆

### 新しい記事の追加

1. `posts/` ディレクトリに新しい `.md` ファイルを作成する
2. frontmatterにタイトル、日付、カテゴリ、説明を追加する
3. 本文をMarkdownで書く
4. ビルド後にホームページに自動表示される

### frontmatterの形式

```markdown
---
title: '記事タイトル'
date: '2024-06-19'
category: 'カテゴリ名'
description: '記事の説明'
---
```

## 作業ログ

- 1つの作業（機能実装、調査、ドキュメント整備など）が完了したタイミングで自動的に作成する
- `docs/worklog/YYYY-MM-DD-{topic}.md` に保存する
- **重要**: 作成前に必ず `date` コマンドで現在の日付を確認する
- ユーザーは読まないので、感じたことをできるだけ率直に書く
- 達成したこと、直面した課題、感情的な変化、技術的な学びを記録する
- **日本語で書く**

### 作業ログのテンプレート

```markdown
# YYYY-MM-DD 作業ログ

## 今日やったこと

- [完了したタスク]

## 直面した課題

- [課題と解決策]

## 感情的な変化

- [個人的な振り返り]

## 技術的な学び

- [技術的な気づき]

## 次回への引き継ぎ

- [次のセッションへのメモ]
```

## ADR（アーキテクチャ決定記録）

- ADRは `docs/adrs/` に保存する
- 新しいADRを作成するときは `docs/adrs/000-template.md` のフォーマットに従う
- 連番で命名する（例: `005-topic-name.md`）

## コミュニケーションガイドライン

- ユーザーとの会話はすべて日本語で行う
- issue, PRは日本語で書く
- コードコメント、コミットメッセージは英語で書く
- `docs/` 以下のドキュメントは日本語で書く

## 参考ドキュメント

- **要件**: 詳細な要件と仕様は `requirements.md` を参照

## 設定

- Claude Codeの権限は `.claude/settings.local.json` で設定
- リポジトリは `main` ブランチで管理
- Astroの設定は `astro.config.ts`
