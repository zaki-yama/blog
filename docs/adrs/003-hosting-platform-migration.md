...
**Astro + Netlify** を採用します。

### 決定理由

1. **技術ブログに最適**: ゼロJSアプローチはブログのパフォーマンスを最大化
2. **優れたパフォーマンス**: Next.jsと比較して90%少ないJS、40%高速
3. **無料で商用利用可能**: Netlifyは個人ブログでの商用利用を許可
4. **移行コストが適度**: Reactコンポーネントも使用可能で段階的な移行が可能
5. **Markdownファースト**: 既存の記事をそのまま活用可能
6. **将来の拡張性**: 必要に応じてインタラクティブ機能を追加可能

### リスク認識

- Astroへの移行作業が必要（デザイン、コンポーネント、ビルド設定）
- Netlifyのビルド時間制限（月300分）に注意が必要
  - 対策: ビルド頻度を調整、必要に応じてプレビューデプロイを制限

### フォールバックプラン

問題が発生した場合の移行先候補：
1. **第一候補**: Astro + Vercel（より多いビルド時間）
2. **第二候補**: Hugo + GitHub Pages（完全無料、最速ビルド）
3. **第三候補**: Next.js + Cloudflare Pages（現状に近い環境）
...
- **Cloudflare Workersの制限から解放**:
  - Workerサイズ3MB → デプロイサイズ実質無制限（83倍以上の関数サイズ）
  - CPUタイム10ms → 10秒〜26秒の実行時間
  - リクエスト制限100,000/日 → 無制限
  - プロジェクト成長時のスケーラビリティ確保
- **パフォーマンス大幅向上**: 90%少ないJS、Lighthouse score 95+
- **ビルド時間短縮**: Next.jsと比較して3倍速いビルド
- **開発体験向上**: Markdownファーストで記事執筆が容易
- **保守性向上**: シンプルなアーキテクチャでメンテナンスが容易

### ネガティブな影響（想定）

- **移行作業**: デザイン、コンポーネント、ビルド設定の移行が必要
- **学習コスト**: Astroの概念（Islands Architecture等）を学ぶ必要
- **ビルド時間制限**: Netlifyの月300分のビルド時間に注意が必要
